...
function joinRoom(){
    ...
    socket.emit('join',param,(err)=>{
        if(err){
            ...
            document.getElementById('roomDetail').innerHTML= '<h3>Room Id - '+saveRoomId+'</h3>'+'<span >Invite link : </span><br><span  id="urlToShare" style="background-color: #1db0ff;margin: 0 auto" >'+urlToShare+'</span><br><button onclick="copyLink()" type="button" class="btn btn-info">copyLink</button>';
        }
    });
}
function copyLink(){
